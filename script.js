...
function generateProductModals(data) {
    ...
    categories.forEach(produit => {
        ...
        //
        // Chaque modale aura un id unique basé sur l'id de chaque produit.
        modal.id = `productModal-${produit.id}`;
        //
        // Ajout class CSS Styles
        modal.className = 'modal product-modal';

        // Vérifie si c'est un menu (menus et Happy Meal) - OU LOGIQUE
...
function calculerTotalCalories(produitId) {
    ...
    if (produit.main) {
        ...
        //
        //
        // le ? fonctionne comme une condition
    ...
    // Ajout de contenu html dans caloriesDisplay
...
window.addEventListener('click', function (event) {
    const modals = document.querySelectorAll('.modal'); // Récupère toutes les modales Classes CSS
    // Parcourir les données avec Foreach
    modals.forEach(modal => {

        if (event.target === modal) { // Si on clique sur l'arrière-plan (et pas le contenu)
            modal.style.display = 'none'; // Ferme la modale
            document.body.classList.remove('modal-open'); // Supprime Class CSS et Réactive le scroll de la page si besoin
        }
    });
});
